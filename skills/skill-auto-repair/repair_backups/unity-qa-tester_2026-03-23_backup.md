...
**코드(.cs)를 수정한 후에는 반드시 플레이 모드를 껐다 켜야 한다.**

```
manage_editor(stop) → [코드 수정] → refresh_unity(compile=request) → manage_editor(play)
```

- 플레이 중인 상태에서 코드를 수정해도, 실행 중인 게임에는 새 코드가 반영되지 않는다
- 컴파일 후 도메인 리로드가 완료된 다음 play를 해야 새 코드가 작동한다
- 코드 수정 없이 씬/에셋만 수정한 경우에는 이 제약이 없다
